...
const postPrint: RouteHandlerMethod = async (request, reply) => {
    try {
        ...
        const lastPrintResult = await request.server.db.fashion.product.selectLastPrint(hex)
        ...
        const result = await request.server.db.fashion.product.insertPrint(insert)
        if (!result) return reply.status(201).send({data: insert});
        else return reply.status(404).send({error: "Cloth not found"})
