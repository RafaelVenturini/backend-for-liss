...
export const testTrans = nodemailer.createTransport(testMailer)
export const fitnessTrans = nodemailer.createTransport(fitnessMailer)


const mailerPlugin: FastifyPluginAsync = async (app: FastifyInstance) => {
    let transporter
    if (appConfig.env === 'prod') transporter = fitnessTrans;
    else transporter = testTrans;
