...
import {ClothsResult, fixImgResult, InsertFitAddress, InsertFitOrder, InsertFitUser,} from "@plugins/interface.js";
import {
    insertColorSql,
    insertMultColorSql,
    insertProductSql,
...
import {
    ...
    insertOrderSql,
    selectAddressSql,
} from "@/lib/database/fitness/order-sql.js";
...
import {
    ...
    selectPrintsSql,
    selectReferencesSql
} from "@/lib/database/fashion/select-react-sql.js";
import {getLastPrintSql, insertPrintSql, insertReferenceSql} from "@/lib/database/fashion/create-product-sql.js"
...
async function databasePlugin(fastify: FastifyInstance) {
    ...
    const insertFitnessAddress = async (address: InsertFitAddress) => {
        ...
        try {
            const [inserted] = await fitnessPool.execute<ResultSetHeader>(
                insertAddresSql,
                insert,
            );
    ...
    const insertFitnessOrderProducts = async (
    ...
    const selectFitnessBrokenImg = async (): Promise<fixImgResult[]> => {
        const [rows] =
            await fitnessPool.execute<fixImgResult[]>(selectBrokenImgSql);
        return rows;
    };
    ...
    const insertFitnessColor = async (name: string, sku: number) => {
        ...
        try {
            if (solid)
                await fitnessPool.execute(insertColorSql, [
                    sku,
                    name,
                    databaseDate(new Date()),
                ]);
            else
                await fitnessPool.execute(insertMultColorSql, [
                    sku,
                    name,
                    databaseDate(new Date()),
                ]);
    ...
    /// Fashion Query's
    ///
    const selectReactCategorys = async () => {
        const [rows] = await fashionPool.execute(selectCategorysSql)
        return rows;
    }

    const selectReactReference = async () => {
        const [rows] = await fashionPool.execute(selectReferencesSql)
        return rows;
    }

    const selectReactCloths = async () => {
        const [rows] = await fashionPool.execute(selectClothsSql)
        return rows;
    }

    const selectReactPrint = async () => {
        const [rows] = await fashionPool.execute(selectPrintsSql)
        return rows;
    }

    const insertPrint = async (item: InsertPrint) => {
        const {est_id, name, tec_id, ref} = item
        const insert = [est_id, name, tec_id, ref]

        try {
            await fashionPool.execute(insertPrintSql, insert)
        } catch (e) {
            return {error: e};
        }
    }

    const insertReference = async (item: InsertReference) => {
        const {sku, ref, name, price, category,} = item
        const insert = [sku, ref, name, price, category]
        try {
            await fashionPool.execute(insertReferenceSql, insert)
        } catch (e) {
            return {error: e};
        }
    }

    const selectLastPrint = async (hex: string | null) => {
        const prefix = hex ? "S" : "E"

        const [rows] = await fashionPool.execute(getLastPrintSql(prefix))
        return rows;
    }
    ...
    fastify.decorate("db", {
        ...
        fashion: {
            pool: fashionPool,
            selectReactCategorys,
            selectReactReference,
            selectReactCloths,
            selectReactPrint,
            insertPrint,
            insertReference,
