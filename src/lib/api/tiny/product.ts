...
import {GetProduct, GetProductsByDate, MultiProduct} from "@api/tiny/interfaces.js";
...
export const getProductsByDate = async (date: string): Promise<MultiProduct[]> => {
    ...
    console.log(`product colect: ${page} / ${data.retorno.numero_paginas} [${percent(page, data.retorno.numero_paginas)}%]`)
